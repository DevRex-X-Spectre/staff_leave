...
import { applyForLeaveAction } from '@/lib/data/actions';
import { listActiveLeaveTypes, getLeaveBalances } from '@/lib/data/dal';
...
export default function ApplyLeavePage() {
  ...
  useEffect(() => {
    listActiveLeaveTypes().then(setLeaveTypes);
  }, []);

  useEffect(() => {
    if (leaveTypeId) {
      getLeaveBalances(leaveTypeId).then(setBalances);
    }
  }, [leaveTypeId]);
  ...
  return (
    <div className="animate-fade-in max-w-2xl mx-auto">
      ...
      {/* Card — single form wrapping all steps */}
      <Card>
        {step === 0 && (
          <>
              ...
              {leaveTypes.map((lt) => (
                ...
                >
                  ...
                  <p className="text-[12px] text-[var(--text-secondary)] mt-1">
                    {lt.max_days_academic ?? lt.max_days_non_academic ?? 0} days max
                  </p>
