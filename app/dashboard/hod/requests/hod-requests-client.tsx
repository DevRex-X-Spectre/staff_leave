...
import { hodDecisionAction } from '@/app/actions/approvals';
...
export function HodRequestsClient({
  ...
  const [howFinanced, setHowFinanced] = useState<'department' | 'applicant'>('department');
  ...
  const handleDecision = (appId: string, decision: 'approve' | 'reject') => {
    ...
    startTransition(async () => {
      const result = await hodDecisionAction({
        applicationId: appId,
        decision,
        comment,
      ...
      if (result.ok) {
        toast.success(decision === 'approve' ? 'Leave approved and forwarded to the Registrar.' : 'Leave rejected.');
      } else {
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        description="Review the complete application details before recording your decision."
      />

      <Card>
        <CardTitle className="mb-4">Pending your review ({applications.length})</CardTitle>

        {applications.length === 0 ? (
          <EmptyState icon={CheckCircle} title="All caught up" description="No pending leave requests in your department." />
        ) : (
          <div className="overflow-x-auto -mx-4 sm:-mx-6 lg:mx-0">
            <table className="w-full">
              <thead>
                <tr className="border-b border-[var(--border-subtle)]">
                  {['Staff', 'Staff ID', 'Rank', 'Leave type', 'Dates', 'Days', 'Conflict', 'Actions'].map((h) => (
                ...
                {applications.map((app) => (
                  <tr key={app.id} className="hover:bg-[var(--bg-hover)] transition-colors">
                    ...
                    <td className="py-3 px-3 sm:px-4">
                      {app.rota_conflict ? <span className="inline-flex items-center gap-1 text-[12px] text-[var(--warning)]"><AlertTriangle size={13} /> Conflict</span> : <span className="text-[12px] text-[var(--text-tertiary)]">-</span>}
                    </td>
                    <td className="py-3">
      ...
      {selectedApp && action && (
        <Dialog
          open
          onClose={closeDialog}
          title={action === 'approve' ? 'Review and approve leave request' : 'Review and reject leave request'}
          description={action === 'approve' ? 'Review the full application, then send it to the Registrar.' : 'Review the full application and state the reason for rejection.'}
          className="max-w-2xl"
        >
          <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {([
                    ...
                    <label key={value} className="flex items-center gap-2 rounded-[var(--radius-md)] border border-[var(--border-subtle)] px-3 py-2.5 text-[13px] text-[var(--text-primary)] cursor-pointer">
                      <input type="radio" name="hod-finance" value={value} checked={howFinanced === value} onChange={() => setHowFinanced(value)} />
                      {label}
                    </label>
                  ))}
                </div>
            ...
            <FormField label={action === 'reject' ? 'Reason (required)' : 'HOD comment (optional)'}>
