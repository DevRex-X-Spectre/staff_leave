...
import { publishRotaAction } from '@/app/actions/rota';
...
export function HodRotaClient({
  ...
  const handleSubmit = () => {
    startTransition(async () => {
      const result = await publishRotaAction({
        departmentId,
        title,
        periodStart,
        periodEnd,
        ...
        slots: slots.map((s) => ({
          ...
          slotEnd: s.slot_end,
          leaveTypeId: s.leave_type_id || null,
        })),
      });
      if (result.ok) {
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        actions={
          <Button variant="ink" onClick={handleSubmit} disabled={isPending}>
            <CalendarRange size={15} />
            {isPending ? 'Publishing...' : 'Publish rota'}
          </Button>
        }
      />
